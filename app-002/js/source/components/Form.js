...
class Form extends Component {
    ...
    render() {
        return <form className="Form">{this.props.fields.map(field => {
            ...
            return (<div className="FormRow" key={field.id} >
                <span className="FormLabel">{field.label}:</span>{field.type === 'rating' ?
                <Rating readonly={true} defaultValue={parseInt(prefilled, 10)} max={11} /> :
                <div>{prefilled}</div>}
            </div>);
        }, this)}</form>;
    }
